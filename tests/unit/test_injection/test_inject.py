...
def test_inject_with_multiple_injections(_create_container: Container) -> None:
    ...
    @inject
    def _injected_func(
        to_inject: int = Injected(_create_container.provider), to_inject_2: int = Injected(_Container.provider_2)
    ) -> int:
...
def test_inject_with_single_injection_and_args(_create_container: Container) -> None:
    ...
    def _injected_func(
        a: int,
        to_inject: int = Injected(_create_container.provider),
    ) -> int:
        return to_inject + a
...
def test_inject_with_nested_injections(_create_container: Container) -> None:
    ...
    @inject
    def _injected_func_2(
        to_inject: int = Injected(_create_container.provider),
        to_inject_2: int = Injected(_Container.provider_2),
        to_inject_3: int = Injected(_create_container.provider),
    ) -> int:
